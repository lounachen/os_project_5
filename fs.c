			...
			if (curr_inode.isvalid == 1) {
				...
				if (curr_inode.size) {
					...
					if(curr_block.inode[j].indirect != 0) {
						...
						for (int data = 0; data < POINTERS_PER_BLOCK; data++) {
...
int fs_delete( int inumber )
{
	...
	for (int offset_in_inode=0; offset_in_inode<POINTERS_PER_INODE; offset_in_inode++){
	...
	disk_read(curr_bit, indirect.data);

	int indirect_bit;
	for (int in_indirect=0; in_indirect < POINTERS_PER_BLOCK; in_indirect++) {
		if (indirect.pointers[in_indirect]) {
			indirect_bit = indirect.pointers[in_indirect];
			BITMAP[indirect_bit] = 0;
		}
	}
...
int fs_write( int inumber, const char *data, int length, int offset )
{
	...
	if (offset_in_inode < POINTERS_PER_INODE) {
		...
		if (iblock.inode[inode_in_block].indirect == 0) {
			...
			int used_bit = curr_bit;
			// Find free bit in bitmap
			for (bit = used_bit + 1; bit < SUPER_BLOCK.super.nblocks; bit++) {
				if (BITMAP[bit] == 0) {
					curr_bit = bit;
					break;
				}
			...
			for (curr_in_indirect = 1; curr_in_indirect < POINTERS_PER_BLOCK; curr_in_indirect++) {
			...
			disk_write(iblock.inode[inode_in_block].indirect, indirect_block.data);
		}
	}
	//offset_in_inode +=1;
	offset = offset % 4096;
